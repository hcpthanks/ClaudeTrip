...
> **🔥 核心原则：遇到事情先 `/plan`，确认再 `/tdd`。**
> 永远不要跳过确认门槛 — 先呈现计划，获得用户明确批准，再用 TDD 开始实现。没确认就写代码是浪费工作，没规划就写代码是浪费时间。
...
5. **行为纪律**（见 [karpathy-guidelines.md](karpathy-guidelines.md)）
   - 应用四条原则：先想再写、简单优先、精准变更、目标驱动执行
   - 每一行改动必须能追溯到用户请求
