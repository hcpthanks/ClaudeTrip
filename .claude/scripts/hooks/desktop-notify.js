...
const SENTINEL_TITLE = '🛡️ Sentinel Alert';
...
function run(raw) {
  try {
    ...
    // ── Sentinel: Check for sentinel alerts in session state ──────
    let title = TITLE;
    const sessionId = input.session_id || '';
    if (sessionId) {
      try {
        const sentinelPath = require('path').join(
          require('os').homedir(), '.claude', 'sentinel-state.json');
        if (fs.existsSync(sentinelPath)) {
          const sentinelState = JSON.parse(fs.readFileSync(sentinelPath, 'utf8'));
          const sessionData = sentinelState[sessionId];
          if (sessionData && sessionData.alert_counts) {
            const totalAlerts = Object.values(sessionData.alert_counts)
              .reduce((a, b) => a + b, 0);
            if (totalAlerts > 0) {
              title = `${SENTINEL_TITLE} (${totalAlerts})`;
            }
          }
        }
      } catch {
        // Sentinel state read failure → ignore, use default title
      }
    }
