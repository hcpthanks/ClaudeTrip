...
(function () {
  ...
  var QUIZ_DATA = {
    ...
    '21-day-plan': {
      handwritingContent: [
        '标准开发流六步：/plan做计划 → /tdd测试驱动 → /code-review审查 → /build-fix修构建 → /verify验证 → /save-session保存',
        '上下文窗口200K tokens，MCP开多了严重占用空间——/context-budget查看用量，/compact压缩，/checkpoint存档防丢失',
        '模型选择：简单任务用Haiku（便宜快）、日常开发用Sonnet（性价比最高）、复杂架构才用Opus（推理最强）',
        '21天覆盖的核心命令：/plan、/tdd、/code-review、/build-fix、/verify、/save-session、/learn-eval、/checkpoint、/context-budget、/docs、/refactor-clean、/pr'
      ],
      questions: [
        { q: '标准开发流的正确顺序是什么？',
          opts: ['/code-review → /plan → /tdd', '/plan → /tdd → /code-review → /verify → /save-session', '/tdd → /plan → /save-session', '随便哪个先都行'],
          ans: 1 },
        { q: '上下文快满了（超过80%），应该做什么？',
          opts: ['不管它继续写', '敲 /compact 压缩，或 /save-session 保存后新开对话', '重启电脑', '多装几个MCP'],
          ans: 1 },
        { q: '想让AI帮忙写个简单的批处理脚本，用哪个模型最划算？',
          opts: ['必须用Opus', 'Haiku——简单任务用便宜的，效果一样', '只能用Sonnet', '随便选'],
          ans: 1 },
        { q: '/learn-eval 和 /save-session 的区别是什么？',
          opts: ['没区别，随便用哪个', '/save-session保存对话进度（一次性），/learn-eval提取经验教训（永久跨对话）', '都是存文件', '都不重要'],
          ans: 1 },
        { q: '代码写完了，提交之前最后一步应该做什么？',
          opts: ['直接关掉', '/verify 做全面检查——构建、测试、类型全跑一遍', '发朋友圈庆祝', '什么都不做直接提交'],
          ans: 1 }
      ]
    }
  };
